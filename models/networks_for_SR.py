...
def define_net(net_type):
    ...
    if net_type == 'EncoderDepth':
        net = EncoderDepth(input_nc=1, ngf=8, norm_layer=nn.BatchNorm2d, use_bias = False, use_dropout=False,  padding_type='reflect',  down=5)
    elif net_type == 'EncoderImage':
        net = DenseNet(start_dims=32, growth_rate=32, reduction=0.5, bottleneck=True, dropRate=0.0, ns=[6,12,16,16])
    elif net_type == 'Decoder':
        net = Decoder(norm_layer=nn.BatchNorm2d, use_bias = False,  up_layers=5, start_dim = 32,  growth_rate=32, ns = [6,12,16,16], dims_d = [8, 16, 32, 64, 128, 256], reduction=0.5)
    else:
...
class EncoderDepth(nn.Module):
    def __init__(self, input_nc, ngf=8, norm_layer=nn.BatchNorm2d, use_bias = False, use_dropout=False,  padding_type='reflect',  down=5):
        super().__init__()
        ...
        for i in range(down):
            mult*= 2
            new_block = nn.Sequential(nn.Conv2d(ngf * mult//2, ngf * mult, kernel_size=3, stride=1, padding=1, bias=use_bias),
                                      norm_layer(ngf * mult),
                                      nn.ReLU(True),
...
class Decoder(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, use_bias = False,  up_layers=5, start_dim = 32,  growth_rate=32, ns = [6,12,16,16], dims_d = [8, 16, 32, 64, 128, 256], 
                 reduction=0.5):
        super().__init__()
        ...
        for i in range(up_layers-1):
            curr_dim = curr_dim + dims_d[-(i+2)] + dims_d[-(i+2)]
        ...
        curr_dim = curr_dim + dims_d[0] + dims_d[0]
...
class DenseNet(nn.Module):
    def __init__(self, start_dims=32, growth_rate=32,
                 reduction=0.5, bottleneck=True, dropRate=0.0, ns=[6,12,16,16]):
