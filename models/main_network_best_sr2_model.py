...
#             self.optimizers.append(self.optimizer_D)
    ...
    def set_input(self, input):
        ...
        self.syn_depth_hr = input['A_d_hr'].to(self.device)
    ...
    def forward(self, stage='train'):
        ...
        holl_mask = torch.where(self.syn_depth_hr<=self.border, torch.tensor(1).float().to(self.syn_depth_hr.device), torch.tensor(0).float().to(self.syn_depth_hr.device))
        right_mask = holl_mask.clone()
        right_mask[:,:,:-1,:]+=right_mask[:,:,1:,:].clone()
        right_mask[:,:,1:,:]+=right_mask[:,:,:-1,:].clone()
        right_mask[:,:,:,:-1]+=right_mask[:,:,:,1:].clone()
        ...
        right_mask = torch.where(right_mask<1, torch.tensor(1).float().to(self.syn_depth_hr.device), torch.tensor(0).float().to(right_mask.device))
        self.syn_mask_hr = right_mask 
...
#             self.loss_real_norms = self.criterion_task(self.norm_real_r2s*self.real_mask, self.norm_real_pred*self.real_mask) 
        ...
        self.mask_syn_add_holes = self.pred_syn_depth*self.syn_mask_hr*self.mask_syn_add_holes
        
        self.loss_task_syn = self.criterion_task(self.syn_depth_hr*self.syn_mask_hr, self.pred_syn_depth*self.syn_mask_hr) 
