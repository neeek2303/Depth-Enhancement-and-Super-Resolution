...
class CycleGANModel(BaseModel):
    ...
    def backward_D_A(self):
        ...
        self.loss_D_A = self.backward_D_basic(self.netD_A, self.real_image, fake_B)

    def backward_D_B(self):
        """Calculate GAN loss for discriminator D_B"""
        fake_A = self.fake_A_pool.query(self.fake_A)
        self.loss_D_B = self.backward_D_basic(self.netD_B, self.syn_image, fake_A)
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            self.loss_idt_A = self.criterionIdt(self.idt_A, self.syn_image) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed: ||G_B(A) - A||
            self.idt_B = self.netG_B(self.real_A)
            self.loss_idt_B = self.criterionIdt(self.idt_B, self.syn_image) * lambda_A * lambda_idt
        else:
            self.loss_idt_A = 0
            self.loss_idt_B = 0
        ...
        self.loss_cycle_A = self.criterionCycle(self.rec_A, self.syn_image) * lambda_A
        # Backward cycle loss || G_A(G_B(B)) - B||
        self.loss_cycle_B = self.criterionCycle(self.rec_B, self.real_image) * lambda_B
        # combined loss and calculate gradients
