...
class MainNetworkModel(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        if opt.norm_loss:
            self.loss_names+=['syn_norms', 'real_norms']
        ...
        self.model_names = ['G_A_d', 'G_B_d',  'I2D_features', 'Image2Depth',  'Depth_f_syn', 'Task']
...
#                                         not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, opt.replace_transpose, n_down = opt.Depthf_ndown)
        ...
        self.netDepth_f_syn = networks.define_G(3, opt.Depthf_outf, opt.Depthf_basef, opt.Depthf_type, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, opt.replace_transpose, n_down = opt.Depthf_ndown)  
       
        
        task_input_features =  opt.ImageDepthf_outf + opt.Depthf_outf + 3 
...
#         print(self.syn_depth.shape)
        if self.opt.use_image_for_trans:
            ...
            if self.opt.use_rec_as_real_input:
                ...
                real_depth = torch.where(self.real_mask<0.05, torch.tensor(-1).float().to(real_depth.device), r2s) 
        else:
...
#         self.real_depth_by_image = self.real_image
        ...
        if self.opt.use_masked:
            ...
            depth_r_inp =  torch.cat([self.depth_masked, self.real_depth, self.real_depth_by_image], dim=1) 
        else:    
            depth_r_inp =  torch.cat([real_depth,  self.real_depth, self.real_depth_by_image], dim=1) 
        ...
        depth_s_inp =  torch.cat([syn_depth, self.syn2real_depth, self.syn_depth_by_image], dim=1)


        
        syn_task_input = torch.cat([self.netDepth_f_syn(depth_s_inp), image_features_syn, depth_s_inp], dim=1)
        real_task_input = torch.cat([self.netDepth_f_syn(depth_r_inp), image_features_real, depth_r_inp], dim=1)
...
#             self.norm_syn2real = calc_norm(self.syn2real_depth_masked)
...
#             self.loss_syn_norms = self.criterion_task(self.norm_syn, self.norm_syn_pred) 
        ...
        if self.opt.norm_loss:
            ...
            self.norm_syn2real = calc_norm((self.syn2real_depth_masked+1)/2, self.K_A, self.crop_A)
            self.norm_syn_pred = calc_norm((self.pred_syn_depth+1)/2, self.K_A, self.crop_A)

            calc_norm = SurfaceNormals_new()
            self.norm_real = calc_norm((self.real_depth+1)/2, self.K_B, self.crop_B)
            self.norm_real_pred = calc_norm((self.pred_real_depth+1)/2, self.K_B, self.crop_B)
...
#         print(tens.shape, tenss.shape)
        ...
        if self.opt.norm_loss:
            self.loss_G+=self.loss_syn_norms*self.opt.w_syn_norm + self.loss_real_norms*self.opt.w_real_norm 
...
#         self.set_requires_grad([self.netG_A , self.netG_A_d, self.netD_depth], False)  # Ds require no gradients when optimizing Gs
        self.set_requires_grad([self.netG_A_d, self.netG_B_d, self.netI2D_features, self.netImage2Depth ], False)  # Ds require no gradients when optimizing Gs
