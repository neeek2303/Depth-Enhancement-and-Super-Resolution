...
class TranslationModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain and self.opt.lambda_identity_A > 0.0:  # if identity loss is used, we also visualize idt_B=G_A(B) ad idt_A=G_A(B)
        ...
        self.border = -0.97
        if self.isTrain:
            if opt.lambda_identity_A > 0.0:  # only works when input and output images have the same number of channels
...
#                 imageio.imwrite(file, (post(self.fake_B.cpu().detach())*5100).astype(np.uint16))
...
#         print(self.real_depth.shape)
    ...
    def backward_G(self, back=True):
        ...
        lambda_idt_A = self.opt.lambda_identity_A
        lambda_idt_B = self.opt.lambda_identity_B
...
#             self.loss_syn_norms = self.criterion_task(self.norm_syn, self.norm_syn_pred) 
        ...
        if lambda_idt_A > 0:
            ...
            if use_idt_masks:
                mask_real = torch.where(self.real_depth<self.border, torch.tensor(0).float().to(self.real_depth.device),  torch.tensor(1).float().to(self.real_depth.device))
                mask_idn = torch.where(self.idt_A<self.border, torch.tensor(0).float().to(self.idt_A.device), torch.tensor(1).float().to(self.idt_A.device))
...
#                 print(self.criterionIdt(self.idt_A*mask_real*mask_idn, self.real_depth*mask_real*mask_idn) * lambda_B * lambda_idt_B , self.loss_idt_A_norm*self.opt.w_norm_idt, self.criterionIdt(self.norm_idt_A*mask_real*mask_idn, self.norm_syn*mask_real*mask_idn) * lambda_B * lambda_idt_B)
                ...
                self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_depth) * lambda_B * lambda_idt_B + self.loss_idt_A_norm*self.opt.w_norm_idt
            
                
            # G_B should be identity if real_A is fed: ||G_B(A) - A||

            self.loss_idt_B_norm = self.criterionIdt(self.norm_idt_B, self.norm_syn) * lambda_A * lambda_idt_A
            self.loss_idt_B = self.criterionIdt(self.idt_B, self.syn_depth) * lambda_A * lambda_idt_A + self.loss_idt_B_norm*self.opt.w_norm_idt
        else:
            self.loss_idt_A = 0
            self.loss_idt_B = 0
...
#             print(self.criterionCycle(self.rec_B*mask_real*mask_rec, self.real_depth*mask_real*mask_rec) * lambda_B , self.loss_cycle_B_norm*self.opt.w_norm_cycle, self.criterionCycle(self.norm_rec_B*mask_real*mask_rec, self.norm_syn*mask_real*mask_rec) * lambda_B)
        ...
        if self.opt.use_rec_iou_error:
            real_holes = torch.where(self.real_depth<self.border, torch.tensor(1).float().to(self.real_depth.device), torch.tensor(0).float().to(self.real_depth.device))
            rec_holes = torch.where(self.rec_B<self.border, torch.tensor(1).float().to(self.rec_B.device), torch.tensor(0).float().to(self.rec_B.device))
