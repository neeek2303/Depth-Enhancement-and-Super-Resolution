...
def main():
  ...
  if opt.export_depth_images:
    sd.export_depth_images(os.path.join(opt.output_path, 'depth'), folder = opt.filename[-17:-5], frame_skip=opt.frame_skip)
  if opt.export_color_images:
    sd.export_color_images(os.path.join(opt.output_path, 'color'), folder = opt.filename[-17:-5], frame_skip=opt.frame_skip)
  if opt.export_poses:
